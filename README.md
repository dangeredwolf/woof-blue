...
- rewrite shared URLs to reference woof.blue
...
- by default, woof.blue should very similar to the official client
...
woof.blue deployments.
For deer, the intended deployment is with a websever than can serve static files, and reroute to `index.html` as needed. Today [woof.blue](https://woof.blue) is hosted on [cloudflare pages](https://pages.cloudflare.com/).
...
> woof.blue is a community fork, and we'd love to merge your PR!

As a rule of thumb, the best features for woof.blue are ones that have a disproportionately positive impact on the user experience compared to the matinance overhead.
Unlike some open source projects, since woof.blue is a soft fork, any features (patches) we add on top of upstream social-app need to be maintained. For example,
...
Without an overriding motivation, opinionated features should exist behind a toggle that is not enabled by default. This allows woof.blue to cater to as many users as possible.
...
- Change all branding in the repository and UI to clearly differentiate from woof.blue.
...
Self hosting is great! It is our intention that woof.blue is easy to self host and build on your own. If you host your own instance of woof.blue, or make your own builds, please
...
If the issue pertains to infastructure, code, or systems outside the scope of woof.blue, please refer to the
